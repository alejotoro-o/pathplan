...
try:
    ...
    raise ImportError(
        "metaheuropt library is required for GWOPlanner. "
        "Please ensure it is installed and available in your PYTHONPATH."
    )

class GWOPlanner(BaseSolver):
    """
    Path Planning using Grey Wolf Optimizer (GWO) from metaheuropt library.
    ...
    def plan(self, start: Tuple[float, float], goal: Tuple[float, float]) -> Tuple[Optional[List[Tuple[float, float]]], List[Tuple[float, float]]]:
        ...
        gwo_solver = GWO(bounds, pop_size=self.pop_size, max_iter=self.max_iter)
        obj_func = self._create_fitness_function(start, goal)
        
        optimizer = Optimizer(gwo_solver, obj_func, num_runs=1)
        optimizer.run(save_results=False)
        
        best_run = optimizer.get_overall_best("GWO")
        if best_run is None:
            return None, []
